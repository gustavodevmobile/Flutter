...
class _ListaProdutosPageState extends State<ListaProdutosPage> {
  ...
  Widget build(BuildContext context) {
    final produtosController = Provider.of<ProdutoController>(context);
    return Scaffold(
      appBar: AppBar(
                ...
                rows: produtosController.produtos.map((produto) {
                  return DataRow(
                    ...
                    cells: [
                      ...
                      DataCell(
                          Text(produto.valorCompra?.toStringAsFixed(2) ?? '-')),
                      DataCell(Text(produto.valorVenda.toStringAsFixed(2))),
                      DataCell(
                          Text(produto.valorNoPix?.toStringAsFixed(2) ?? '-')),
                      DataCell(Text(
                      ...
                      DataCell(Icon(
                          ...
                          color:
                              produto.freteGratis ? Colors.green : Colors.red)),
                      DataCell(Text(produto.origem ?? '-')),
                      DataCell(Icon(
                          produto.disponivel ? Icons.check : Icons.close,
                          color:
                              produto.disponivel ? Colors.green : Colors.red)),
                      DataCell(Icon(
                          produto.isEletrico
                              ? Icons.electric_bolt
                              : Icons.close,
                          color:
                              produto.isEletrico ? Colors.amber : Colors.grey)),
